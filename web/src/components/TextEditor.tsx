...
interface TextEditorProps {
  ...
  isInserting: boolean;
  /** Extension result from command mode (search results, etc.) */
 ...
 * stable above. Also renders command mode extension results inline.
 */
export function TextEditor({
  ...
  const hasSearchResults =
    commandResult?.type === "search" && commandResult.searchResults;

  // What to actually render in the editor
  const displayStable = showRaw ? rawText : stableText;
    ...
    <div className="w-full max-w-3xl mx-auto flex flex-col flex-1">
      ...
      <div
        ...
        className={`
          ...
          ${hasContent || hasSearchResults ? "text-gray-100" : "text-gray-600"}
          ${isInserting ? "border border-cyan-500/20 bg-cyan-500/[0.02]" : "border border-gray-800/50 bg-gray-950/30"}
          transition-colors duration-300 focus:outline-none
        `}
      >
        {/* Search results (from command mode) */}
        {hasSearchResults && commandResult?.searchResults && (
          <SearchResults results={commandResult.searchResults} />
        )}

        {/* Dictated text */}
        {hasContent ? (
          <p className={`whitespace-pre-wrap ${hasSearchResults ? "mt-6 pt-4 border-t border-gray-800/50" : ""}`}>
            {displayStable}
